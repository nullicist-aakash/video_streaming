...
void ConnectionManager::handle_local_server_to_remote_client(
    const iphdr* ip_header,
    int len, 
    const tcphdr* tcp_header)
{
    SocketPair socket_pair;
    socket_pair.self_ip = 0;
    socket_pair.self_port = 0;
    if (this->rev_remote_to_generated.find(tcp_header->dest) == this->rev_remote_to_generated.end())
    {
        std::clog << "Unknown port generated by local server: " << ntohs(tcp_header->dest) << std::endl;
        return;
    }
    
    socket_pair.peer_ip = 0;
    socket_pair.peer_port = this->rev_remote_to_generated[tcp_header->dest];
}

void ConnectionManager::handle_local_client_to_remote_server(
    const iphdr* ip_header,
    int len, 
    const tcphdr* tcp_header)
{
    SocketPair socket_pair;
    socket_pair.self_ip = 0;
    socket_pair.self_port = tcp_header->source;
    socket_pair.peer_ip = 0;
    socket_pair.peer_port = 0;
}

void ConnectionManager::handle_remote_client_to_local_server(
    const iphdr* ip_header,
    int len, 
    const tcphdr* tcp_header, 
...
{
    SocketPair socket_pair;
    socket_pair.self_ip = inet_addr("127.0.0.1");

    auto source_port_n = tcp_header->source;
    if (this->remote_to_generated.find(source_port_n) == this->remote_to_generated.end())
    {
        this->remote_to_generated[source_port_n] = htons(this->generated_port_counter);
        this->rev_remote_to_generated[htons(this->generated_port_counter)] = source_port_n;
        this->generated_port_counter++;
        this->add_ip_table_entry(source_port_n);
    }

    socket_pair.self_port = this->remote_to_generated[source_port_n];
    socket_pair.peer_ip = socket_pair.self_ip;
    socket_pair.peer_port = 0;
}

void ConnectionManager::handle_remote_server_to_local_client(
    const iphdr* ip_header,
    int len, 
    const tcphdr* tcp_header, 
    const udphdr* udp_header)
{
    SocketPair socket_pair;
    socket_pair.self_ip = inet_addr("127.0.0.1");
    socket_pair.self_port = config.self_mimic_port_n;
    socket_pair.peer_ip = socket_pair.self_ip;
    socket_pair.peer_port = tcp_header->dest;
}
...
void ConnectionManager::handle_packets(int tcp_raw_socket, int udp_raw_socket)
{
    auto handle_packets = [&](const iphdr* ip_header, const int packet_size)
    {
        ...
        if (direction == PacketDirection::LOCAL_SERVER_TO_REMOTE_CLIENT)
            handle_local_server_to_remote_client(ip_header, packet_size, tcp_header);
        else if (direction == PacketDirection::LOCAL_CLIENT_TO_REMOTE_SERVER)
            handle_local_client_to_remote_server(ip_header, packet_size, tcp_header);
